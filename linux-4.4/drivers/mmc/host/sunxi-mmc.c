...
static int sunxi_mmc_extra_of_parse(struct mmc_host *host)
{
	...
	if (of_property_read_bool(np, "cap-wait-while-busy")) {
		...
		if (of_property_read_u32(np,
			...
			host->max_busy_timeout = SUNXI_DEF_MAX_R1B_TIMEOUT_MS;
		} else {
			if (max_busy_timeout < SUNXI_MIN_R1B_TIMEOUT_MS)
				max_busy_timeout = SUNXI_MIN_R1B_TIMEOUT_MS;
			host->max_busy_timeout = max_busy_timeout;
			dev_dbg(host->parent, "max-busy-timeout is %d ms\n",
					max_busy_timeout);
		}
	}
