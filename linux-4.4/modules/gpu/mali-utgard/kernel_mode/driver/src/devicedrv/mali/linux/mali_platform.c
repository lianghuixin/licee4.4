...
void enable_gpu_clk(void)
{
	...
	/*
	 * To prevent multual interference among different modules when closing or
	 * opening PLL clocks, maybe CCMU driver will set PLL clocks to the minimum
	 * value instead of really closing it. So here we reset the clock before
	 * enabling GPU clock gating.
	 */
	set_freq(aw_private.pm.clk[0].freq);
